...
- 실제 생성된 GraphQL 스키마
  ![alt text](/assets/img/blog/image-11.png)
...
### Cannot destructure property 'fieldType' of 'fieldDef' as it is undefined

```ts
import { a } from "@aws-amplify/backend";

export const userModels = {
  DeviceType: a.enum(["APNS", "FCM"]),
  User: a
    .model({
      id: a.id().required(),
      deviceToken: a.string(),
      deviceType: a.ref("DeviceType").required(),
  ...
  getProfile: a
    ...
    // .returns("Profile") <- 문자열이 그대로 들어가면 안돼서 오류
